...
# from BERT.modeling import BertForSequenceClassification, BertConfigWEIGHTS_NAME, CONFIG_NAME
...
class USE(object):
    def __init__(self, cache_path):
        ...
        module_url = 'tf_hub_cache/large-5'
...
class NLI_infer_BERT(nn.Module):
    def __init__(self,
        ...
        output_model_file = os.path.join(target_model_path, WEIGHTS_NAME)
        output_config_file = os.path.join(target_model_path, CONFIG_NAME)
        # Load a trained model and config that you have fine-tuned
        config = BertConfig(output_config_file)
        self.model = BertForSequenceClassification(config, num_labels=nclasses)
        self.model.load_state_dict(torch.load(output_model_file))
        
        self.model.to(device)
        # construct dataset loader
        self.dataset = NLIDataset_BERT(pretrained_dir=bert_model, max_seq_length=max_seq_length, batch_size=batch_size)
...
def attack(fuzz_val, top_k_words, qrs, sample_index, text_ls, true_label,
           ...
           batch_size=32, max_iters=20, target_dataset=None):
    ...
    if true_label != orig_label:
        ...
        print(pos_ls)
        print("-------------")
        #adj:形容词、adv:副词、verb:动词、noun 名词；仅对这些词进行扰动~~
        if 'mr' in target_dataset or 'weibo' in target_dataset:
            pos_pref = {"ADJ": 0, 'ADV': 1, "VERB": 2, "NOUN": 3}
        elif 'ag' in target_dataset:
            # pos_pref = {"NOUN": 0}
            pos_pref = {"ADJ": 0, 'ADV': 1, "VERB": 2, "NOUN": 3}
        else:
            pos_pref = {"ADJ": 0, 'ADV': 1, "VERB": 2, "NOUN": 3}
        ...
        while qrs < old_qrs + 2500 and flag == 0:
            ...
            if np.sum(pr)>0:
                flag = 1
                break
        # print('[step 1 cost time]:', time.time()-stime)
        ...
        if flag == 1:
            ...
            # print('[Step 2 cost time]:', time.time() - stime)
            population_size = 32
            population = []
            old_syns = {}
            ...
            # print('[Step 3 cost time]:', time.time()-stime)
            # print('start for Genetic Optimization')
            pop_count = 0
            attack_same = 0
...
def main():
    ...
    parser.add_argument("--max_iters",
                        type=int,
                        default=20,
                        help="How many genetic iterations for adversarial dataset...")
    ...
    # get data to attack
    if 'mr' in args.dataset_path:
        texts, labels = dataloader.read_corpus('data/mr_all/dev.csv', clean=False, MR=True, shuffle=False)
    elif 'ag' in args.dataset_path:
        texts, labels = dataloader.read_corpus('data/ag_news_all/test.csv', csvf=True, clean=True,
                                                del_stop=True,
                                                MR=False, shuffle=False)
    elif 'weibo2018' in args.dataset_path:
        texts, labels = dataloader.read_weibo_corpus('data/weibo2018/test.txt', shuffle=False)
    elif 'tnews' in args.dataset_path:
        texts, labels = dataloader.read_tnews_corpus(data_path="data/tnews/dev.json",
    ...
    if args.target_model == 'wordLSTM':
        ...
        model = NLI_infer_BERT(target_model_path=args.target_model_path,
    ...
    if args.counter_fitting_cos_sim_path!='':
        ...
        sim_lis = np.dot(embeddings, embeddings.T)
        '''
        _, embeddings = dataloader.load_embedding("data/sgns.weibo.word.bz2")
        embeddings = np.array(embeddings)
        print(embeddings.T.shape)
        norm = np.linalg.norm(embeddings, axis=1, keepdims=True)
        embeddings = np.asarray(embeddings / norm, "float64")
        sim_lis = np.dot(embeddings, embeddings.T)
        pickle.dump(sim_lis, open('data/chinese-counter-fitted-vectors.txt','wb'))
    ...
    for idx, (text, true_label) in tqdm(enumerate(data)):
        ...
        all_true_ids.append(true_label)
        ...
        # print(orig_label.data.cpu().numpy())
        all_orig_ids.append(orig_label.data.cpu().numpy())
        all_new_ids.append(new_label.data.cpu().numpy())
    ...
    print('orig acc:', accuracy_score(y_true=all_true_ids, y_pred=all_orig_ids))
    print(classification_report(y_true=all_true_ids, y_pred=all_orig_ids))
    print('-------------------------')
    
    print('adv acc:', accuracy_score(y_true=all_true_ids, y_pred=all_new_ids))
    print(classification_report(y_true=all_true_ids, y_pred=all_new_ids))
    print('-------------------------')
...
#        csvwriter.writerows(fails)
    ...
    with open(os.path.join(args.output_dir, args.output_name), 'w') as ofile:
