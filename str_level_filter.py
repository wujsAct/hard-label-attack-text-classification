...
def clean_text(text):
    text = URL_REGEX.sub(' ', text) #url删除
    text = PHONE_REGEX.sub(' ', text)#phone
    text = HASHTAG_REGEX.sub(' ',text) #hashtag
    text =ALPHA_NUM_REGEX.sub(' ',text)#non
    
    
    ##删除表情
    rets = remove_emoji3(text)
    # print(rets)
